    ...
    <form action="post.php" method="post">
      <div class="form-group">
        ...
        <?php
          ...
          for($x = 0; $x < $length; $x++){
            if ($company->getIntersection()->getId() === $intersections[$x]->getId()){
      ...
      <div class="form-group">
        <label for="deliveryBaseCost">Delivery Base Cost</label>
        <?php
          echo "<input type='text' class='form-control' name='deliveryBaseCost' value='".$company->getDeliveryBaseCost()."'>";
        ?>
      </div>
      <div class="form-group">
        <label for="deliveryPerBlockCost">Delivery Per Block Cost</label>
        <?php
          echo "<input type='text' class='form-control' name='deliveryPerBlockCost' value='".$company->getDeliveryPerBlockCost()."'>";
        ?>
      </div>
      <div class="form-group">
        <label for="courierBonusTime">Courier Bonus Time</label>
        <?php
          echo "<input type='text' class='form-control' name='courierBonusTime' value='".$company->getCourierBonusTime()."'>";
        ?>
      </div>
      <div class="form-group">
        <label for="courierBonusAmount">Courier Bonus Amount</label>
        <?php
          echo "<input type='text' class='form-control' name='courierBonusAmount' value='".$company->getCourierBonusAmount()."'>";
        ?>
      </div>
      <button type="submit" class="btn btn-primary" name="submitEditCompany">Update</button>
    </form>
  </div>
</body>
</html>
