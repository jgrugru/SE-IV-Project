...
<body>
  ...
  <div class="container">
    <h3 style>User</h3>
    <a href="addUser.php"><button type="button" class="btn btn-default" style="margin-bottom:10px">Add User</button></a>
    <table class="table table-bordered table-striped table-hover" id="myTable">
      ...
      <?php
        $length = count($users);
        for($x = 0; $x < $length; $x++){
          $employee = $db->getEmployeeById($users[$x]->getEmployeeId());
          echo "<tr class='table-row' onclick='route(".$users[$x]->getEmployeeId().")'>";
          echo "<td>".($x+1)."</td>";
          echo "<td>".$users[$x]->getEmployeeId()."</td>";
          echo "<td>".$employee->getFirstName()." ".$employee->getLastName()."</td>";
          echo "<td>".$users[$x]->getPermission()."</td>";
          echo "</tr>"; 
        }
      ?>
      </tbody>
